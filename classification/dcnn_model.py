...
class DCNN_Model:
    ...
    def check_model_exist(self):
        if os.path.exists(self.path):
            ...
            image_id_path_dict, df = self.preprocessing_data(df)
            features = df.drop(columns=['cell_type_idx'], axis=1)
            target = df['cell_type_idx'] # Trying to make

            # Image process for features and the targets
            self.image_preprocessing(features, target)
            self.load_model()
    ...
    def modelling(self, df):
        ...
        # Fitting the model
        epochs = 10 # For temporary 10 so I can debug better
        batch_size = 10
    ...
    def predict(self, image_path):
        print(self.features_std, self.features_mean)
        ...
        print('Result : ', class_idx_to_label[result])
        return result
    ...
    def preprocess_input_image(self, image_path):
        ...
        img = (img - self.features_mean) / self.features_std  # Normalize the image
    ...
    def image_preprocessing(self, features, target):
        ...
        self.features_mean = np.mean(features)
        self.features_std = np.std(features)
